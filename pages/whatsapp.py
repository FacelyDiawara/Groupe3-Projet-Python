...
class WhatsAppPage(QWidget):
    ...
    def init_ui(self):
        self.setObjectName("WhatsAppPage")
        self.setStyleSheet("""
            QWidget#WhatsAppPage {
        ...
        title = QLabel("Génération WhatsApp")
        ...
        # Input Group
        form_group = QGroupBox("Nouveau Message WhatsApp")
        form_group.setFont(QFont("Times New Roman", 15, QFont.Bold))
        grid_layout = QGridLayout()
        grid_layout.setSpacing(15)

        self.phone_input = QLineEdit()
        self.phone_input.setPlaceholderText("Numéro WhatsApp (ex: +224...)")
        self.phone_input.setReadOnly(False) # Permission d'éditer le numéro
        self.phone_input.setStyleSheet("background-color: white; border: 2px solid #25d366;")
        ...
        self.message_input.setPlaceholderText("Message WhatsApp...")
        
        send_btn = QPushButton("✅ VALIDER L'ORDONNANCE & ENVOYER")
        send_btn.clicked.connect(self.send_whatsapp)
        ...
        top_row_layout.addWidget(QLabel("Numéro:"))
    ...
    def load_active_reminders(self):
        ...
        for rem in reminders:
            if rem.get("type_canal") != "WhatsApp":
                continue
    ...
    def send_whatsapp(self):
        ...
        if number and msg:
            ...
            # 3. Real Messaging (WhatsApp)
            success, sid_or_error = MessagingService.send_whatsapp(number, msg)
            
            if success:
                print(f"Rappels WhatsApp programmés pour {self.patient_nom} pendant {validity} jours. SID: {sid_or_error}")
                QMessageBox.information(
                    self, "Succès", 
                    f"Ordonnance validée.\n\nLe message WhatsApp a été envoyé à {number} et les rappels ont été programmés."
                )
            else:
                QMessageBox.warning(
                    self, "Erreur Envoi", 
                    f"Ordonnance validée mais le message WhatsApp n'a pas pu être envoyé.\nErreur: {sid_or_error}\n\nAssurez-vous d'avoir configuré des identifiants Twilio valides dans config.py."
                )
